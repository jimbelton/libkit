...
#   define ADDR_FROM      ((struct sockaddr *)&addr_from)
    ...
    is(kit_recvfrom(fd_recv, buffer, sizeof(buffer), MSG_DONTWAIT, ADDR_FROM, &addr_len, &delay_in_msec, &ttltos),
    ...
    is(kit_recvfrom(fd_recv, buffer, sizeof(buffer) / 2, MSG_DONTWAIT, ADDR_FROM, &addr_len, &delay_in_msec, NULL),
    ...
    is(kit_recvfrom(fd_recv, buffer, sizeof(buffer) / 2, MSG_DONTWAIT | MSG_TRUNC, ADDR_FROM, &addr_len, &delay_in_msec, NULL),
    ...
    is(kit_recvfrom(fd_recv, buffer, sizeof(buffer), MSG_DONTWAIT, ADDR_FROM, &addr_len, &delay_in_msec, NULL),
    ...
    diag("Test limits");
    ok((fd_recv = kit_udp_socket(AF_INET, KIT_UDP_DELAY | KIT_UDP_TTLTOS)) >= 0, "Created recv socket asking for all the things");
    addr_to.sin_family = AF_INET;
    addr_to.sin_port = 0;
    ok(inet_aton("127.0.0.1", &addr_to.sin_addr), "Created IP address");
    ...
    is(kit_recvfrom(fd_recv, buffer, TEST_MESSAGE_SIZE, MSG_DONTWAIT, ADDR_FROM, &addr_len, &delay_in_msec, &ttltos),
       TEST_MESSAGE_SIZE, "Received %zu bytes", TEST_MESSAGE_SIZE);
