...
main(void)
{
    ...
    is_strncmp(str, deviceidstr, strlen(str), "Printed string matches expected");

    ok(str = kit_deviceid_to_str(NULL), "Called kit_deviceid_to_str with NULL");
    is_strncmp(str, "0000000000000000", strlen(str), "Printed string for NULL matches expected");
    ...
    is_strncmp(kit_deviceid_to_str(&deviceid2), deviceidstr, KIT_DEVICEID_STR_LEN, "Print of build deviceid as expected");
    ...
    is_strncmp(str, "0000000000000000", strlen(str), "Printed string matches expected");
