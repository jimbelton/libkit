...
unsigned char testa_md5[] = {0x90, 0x01, 0x50, 0x98, 0x3c, 0xd2, 0x4f, 0xb0, 0xd6, 0x96, 0x3f, 0x7d, 0x28, 0xe1, 0x7f, 0x72};
unsigned char testb_md5[] = {0x82, 0x15, 0xef, 0x07, 0x96, 0xa2, 0x0b, 0xca, 0xaa, 0xe1, 0x16, 0xd3, 0x87, 0x6c, 0x66, 0x4a};
unsigned char testc_md5[] = {0x77, 0x07, 0xd6, 0xae, 0x4e, 0x02, 0x7c, 0x70, 0xee, 0xa2, 0xa9, 0x35, 0xc2, 0x29, 0x6f, 0x21};
...
{
    if (memcmp(expected, got, SXE_MD5_SIZE) == 0) {
        pass("%s", str);
        return;
    }

    diag("Expected MD5:");
    SXED1(expected, SXE_MD5_SIZE);
    diag("Got      MD5:");
    SXED1(got, SXE_MD5_SIZE);
    fail("%s", str);
}
...
main(void)
{
    char Message_Digest_Hex[SXE_MD5_IN_HEX_LENGTH + 1];
    uint8_t Message_Digest[SXE_MD5_SIZE];
    SXE_MD5 md5;
    int i;
     ...
     */
    SXE_MD5_Init(  &md5);
    SXE_MD5_Update(&md5, (const unsigned char *)TESTA, strlen(TESTA));
    SXE_MD5_Final(Message_Digest, &md5);
    is_md5(testa_md5, Message_Digest, "Test A: MD5 as expected");
    sxe_md5_to_hex(Message_Digest, Message_Digest_Hex, sizeof(Message_Digest_Hex));
     ...
     */
    SXE_MD5_Init(  &md5);
    SXE_MD5_Update(&md5, (const unsigned char *)TESTB, strlen(TESTB));
    SXE_MD5_Final(Message_Digest, &md5);
    ...
    for(i = 1; i <= 1000000; i++) {
        SXE_MD5_Update(&md5, (const unsigned char *)TESTC, 1);
    }
     ...
     */
    SXE_MD5_Init(  &md5);
    SXE_MD5_Update(&md5, (const unsigned char *)TESTQ, strlen(TESTQ));
    SXE_MD5_Final(Message_Digest, &md5);
     ...
     */
    SXE_MD5_Init(  &md5);
    SXE_MD5_Update(&md5, (const unsigned char *)TESTL, strlen(TESTL));
    SXE_MD5_Final(Message_Digest, &md5);
